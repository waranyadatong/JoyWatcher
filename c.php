...
<?php include_once('layouts/header.php');?>
   ...
   <form class="form-inline" action="#">
      ...
      <button  onclick="tablesToExcel( [{ 'sheet': 'Overview','tables' : ['tbl_exporttable_to_xls_1','tbl_exporttable_to_xls_2']}], 'Analytics_JoyWatcher.xls', 'Excel')"class="button button2" class="form-control" name="excel"><span class="glyphicon glyphicon-floppy-save"></span> Export to Excel</button>
      <?php } ?>   
   </form>
   <br/>
   <?php
      ...
      if(isset($_GET['from_date']) && isset($_GET['to_date'])) {      
         ...
         $query = "SELECT count(*) AS total FROM a3 WHERE Date_Time BETWEEN '$from_date $Timeoutput' AND '$to_date $Timeoutput'";
         ...
         $querytime = "SELECT Date_time AS t FROM a3 WHERE Date_Time BETWEEN '$from_date $timestart' AND '$to_date $timestart'";
         ...
         $queryavg = "SELECT Takt_time as takttime,start_time as starttime,end_time as endtime FROM a3 WHERE Date_Time BETWEEN '$from_date $timeavg' AND '$to_date $timeavg'";
         ...
         $queryrunstop = "SELECT Takt_time as tk,start_time as st,end_time as et FROM a3 WHERE Date_Time BETWEEN '$from_date $timerunstop' AND '$to_date $timerunstop'";
         ...
         $dataPoints1 = array(
            array("y" => $pruntime, "label" => "MOT A3")
         );
         $dataPoints2 = array(
            array("y" => $pstoptime, "label" => "MOT A3")
         ); 
   }else{
         $dataP1 = 0;
         $dataP2 = 0;
         $dataPoints1 = array(
            array("y" => $dataP1, "label" => "MOT A3")
         );

         $dataPoints2 = array(
            array("y" => $dataP2, "label" => "MOT A3")
         );
                  ...
                  <h3 class="panel-title" style="margin-top: 4px; font-size: 19px; text-align: center;">Data Output of Joy Watcher
                     <form class="pull-right" action="" enctype="multipart/form-data">
                        <!--<button onclick="downloadImage1();" class="button button3" style="margin-top: -10px; margin-right: -5px; font-weight: normal;"><span class="glyphicon glyphicon-floppy-save"></span>Export File</button>-->
                            <a data-name="myChart" id="downloadavg" download="ChartOutput.jpg" href="" class="download button button3"class="download glyphicon glyphicon-floppy-save"style="margin-top: -10px; margin-right: -5px; font-weight: normal;">Export File</a>
                     </form>
   ...
   var chart1 = new CanvasJS.Chart("chartMachineRuntime",
      {
         ...
         data: [
         {
            ...
            dataPoints: <?php echo json_encode($dataPoints1, JSON_NUMERIC_CHECK); ?>
               //{ y:'pruntime' , label: "MOT A3", indexLabel: "pruntime" }
      ...
      $("#exportButton").click(function(){
         html2canvas(document.querySelector("#chartMachineRuntime")).then(canvas => { 
            ...
            downloadLink.setAttribute('download', 'MachineRuntime.png');
